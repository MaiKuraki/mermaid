...
class RailroadLexer {
  ...
  private previousTokenEndsFactor(): boolean {
    return (
      this.previousTokenType === TokenType.DOUBLE_STRING ||
      this.previousTokenType === TokenType.SINGLE_STRING ||
      this.previousTokenType === TokenType.IDENTIFIER ||
      this.previousTokenType === TokenType.SPECIAL ||
      this.previousTokenType === TokenType.RIGHT_PARENTHESIS ||
      this.previousTokenType === TokenType.RBRACKET ||
      this.previousTokenType === TokenType.RBRACE
    );
  }

  private hasClosingSpecialSequence(): boolean {
    for (let offset = 1; this.position + offset < this.input.length; offset++) {
  ...
  private canReadSpecial(hadLeadingWhitespace: boolean): boolean {
    return (
      (hadLeadingWhitespace || !this.previousTokenEndsFactor()) && this.hasClosingSpecialSequence()
    );
  }
  ...
  nextToken(): Token {
    let hadLeadingWhitespace = this.skipWhitespace();
    ...
    if (this.position >= this.input.length) {
      return this.emit({ type: TokenType.EOF, value: '', line, column });
    }
    ...
    // Strings
    if (char === '"') {
      return this.emit(this.readString('"'));
    }
    if (char === "'") {
      return this.emit(this.readString("'"));
    }
    ...
    // Special sequence
    if (char === '?' && this.canReadSpecial(hadLeadingWhitespace)) {
      return this.emit(this.readSpecial());
    }
    ...
    if (char === ':' && this.peek(1) === ':' && this.peek(2) === '=') {
      ...
      this.advance();
      this.advance();
      return this.emit({ type: TokenType.BNF_DEFINE, value: '::=', line, column });
    }
    ...
    switch (char) {
      case '=':
        this.advance();
        return this.emit({ type: TokenType.EQUALS, value: '=', line, column });
      case ';':
        this.advance();
        return this.emit({ type: TokenType.SEMICOLON, value: ';', line, column });
      case '|':
        this.advance();
        return this.emit({ type: TokenType.PIPE, value: '|', line, column });
      case '?':
        this.advance();
        return this.emit({ type: TokenType.QUESTION, value: '?', line, column });
      case '*':
        this.advance();
        return this.emit({ type: TokenType.STAR, value: '*', line, column });
      case '+':
        this.advance();
        return this.emit({ type: TokenType.PLUS, value: '+', line, column });
      case '(':
        this.advance();
        return this.emit({ type: TokenType.LEFT_PARENTHESIS, value: '(', line, column });
      case ')':
        this.advance();
        return this.emit({ type: TokenType.RIGHT_PARENTHESIS, value: ')', line, column });
      case '[':
        this.advance();
        return this.emit({ type: TokenType.LBRACKET, value: '[', line, column });
      case ']':
        this.advance();
        return this.emit({ type: TokenType.RBRACKET, value: ']', line, column });
      case '{':
        this.advance();
        return this.emit({ type: TokenType.LBRACE, value: '{', line, column });
      case '}':
        this.advance();
        return this.emit({ type: TokenType.RBRACE, value: '}', line, column });
      case ',':
        this.advance();
        return this.emit({ type: TokenType.COMMA, value: ',', line, column });
      case '-':
        this.advance();
        return this.emit({ type: TokenType.MINUS, value: '-', line, column });
    }

    // Identifiers
    if (/[A-Z_a-z]/.test(char)) {
      return this.emit(this.readIdentifier());
    }
