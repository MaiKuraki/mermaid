  ...
  for (const edge of edges) {
    ...
    if (srcRect) {
      const snapped = snapEndpointToBoundary(
        firstDistinctAdjacent(newPts, 0, 1),
        newPts[0],
        srcRect
      );
      if (snapped !== newPts[0]) {
      ...
      newPts = collapseOwnBorderStub(newPts, srcRect, true);
    }
    if (dstRect) {
      const last = newPts.length - 1;
      const snapped = snapEndpointToBoundary(
        firstDistinctAdjacent(newPts, last, -1),
        newPts[last],
        dstRect
      );
      ...
      newPts = collapseOwnBorderStub(newPts, dstRect, false);
    }
...
function separateSharedRenderedTerminalLanes(edges: any[], nodeByIdMap: Map<string, any>): void {
  const EPS_LOCAL = 1e-3;
  const MIN_SHARED = 8;
  const TRACK_SHIFT = 7;
...
function collapseRedundantRectangularDoglegs(edges: any[], nodeByIdMap: Map<string, any>): void {
  const EPS_LOCAL = 1e-3;
  const MIN_SHARED = 8;
  const BUFFER = 2;
